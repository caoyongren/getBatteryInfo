...
class BatterySipper implements Comparable<BatterySipper> {
	...
	public BatterySipper(Context context, String pkgName, double time) {
		mContext = context;
		value = time;
		drainType = BatteryInfo.DrainType.APP;
		getQuickNameIcon(pkgName);
	}

	public BatterySipper(Context context, BatteryInfo.DrainType type, BatteryStats.Uid uid, double[] values) {
		mContext = context;
		this.values = values;
		this.drainType = type;
		if (values != null)
	...
	private void getQuickNameIconForUid(BatteryStats.Uid uidObj) {
