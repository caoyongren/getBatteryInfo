...
//		mBatteryInfo = IBatteryStats.Stub.asInterface(ServiceManager.getService("batteryinfo"));
		...
		try {
	        ...
	        Object oRemoteService = mGetService.invoke(null, "batteryinfo");
	        mBatteryInfo = IBatteryStats.Stub.asInterface((IBinder) oRemoteService);
...
//					totalTime += time;
//				}
				if (templist.containsKey(pkgNames[0])) {
					BatterySipper sipper = templist.get(pkgNames[0]);
					sipper.setValue(sipper.getValue() + time);
				} else {
					templist.put(pkgNames[0], new BatterySipper(mContext, pkgNames[0], time));
				}
				totalTime += time;
			}
		}
		for (RunningServiceInfo info : runningService) {
			...
			if(Utils.isSystemApp(mContext, info.service.getPackageName())) continue;
	...
	private void processAppUsage() {
		...
		for (int iu = 0; iu < NU; iu++) {
			...
			for (Map.Entry<Integer, ? extends BatteryStats.Uid.Sensor> sensorEntry : sensorStats.entrySet()) {
				BatteryStats.Uid.Sensor sensor = sensorEntry.getValue();
			...
			int tBluetoothUid = -1;
			try {
				tClazz = Class.forName("android.os.Process");
				tWifiUid = tClazz.getDeclaredField("WIFI_UID").getInt(null);
				tBluetoothUid = tClazz.getDeclaredField("BLUETOOTH_GID").getInt(null);
			} catch (ClassNotFoundException e) {
				e.printStackTrace();
			} catch (IllegalAccessException e) {
				e.printStackTrace();
			...
			if (power != 0) {
				...
				if (u.getUid() == tWifiUid) {
					mWifiSippers.add(app);
				} else if (u.getUid() == tBluetoothUid) {
